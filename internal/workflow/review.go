...
func (service *ReviewService) Review(ctx context.Context, controllerRoot, workflowID string, checkResults []checks.Result, checkedState gitrepo.WorktreeBaseline) (ReviewResult, error) {
	...
	if current.Phase != state.PhaseImplementation || current.Status != state.StatusRunning || current.SpecificationPath == "" || current.Review != nil {
	...
	for {
		...
		outcome, correctionRun, err := service.runCorrection(ctx, controllerRoot, absoluteWorktree, outputDirectory, correctionData)
...
func (service *ReviewService) runCorrection(ctx context.Context, controllerRoot, worktree, outputDirectory string, data prompt.PromptData) (agent.Outcome, agent.RunResult, error) {
	promptText, err := service.correctionPrompt.Render(data)
	if err != nil {
