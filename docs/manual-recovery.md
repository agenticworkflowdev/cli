...
- For `recon/failed`, preserve the generated worktree, `recon.md` when present,
  and error log. Correct an external agent-CLI, timeout, schema, or filesystem
  problem, then inspect the partial artifact if one exists. There is no
  automated retry for this phase.
- For `spec/failed`, preserve `recon.md`, the generated worktree, and the error
  log. There is no automated retry for this phase.
